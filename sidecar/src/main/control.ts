...
export function resolveControlToken(
	...
	return resolveSidecarToken(env, "RYU_SIMULATOR_TOKEN");
}
...
/** Reject drive-by browser requests and DNS rebinding before reading a body. */
...
/** Only explicit JSON bodies are accepted by the simulator action routes. */
...
export function startControlServer(deps: RequestDeps, port: number): Server {
	const server = createServer((req, res) => {
		const requestPort = req.socket.localPort ?? port;
		if (!isTrustedLocalRequest(req, requestPort)) {
		...
		const isPublicHealth = method === "GET" && path === "/health";
		if (!(isPublicHealth || bearerOk(req.headers.authorization, deps.token))) {
